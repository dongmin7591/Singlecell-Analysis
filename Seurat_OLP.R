...
png("./png/variable plot.png",width=6000,height=3500,res=500)
plot1 + plot2
dev.off()
...
p2 <- DimPlot(seurat.integrated, reduction = 'umap', group.by = 'Group')
...
sweep.res.list_nsclc <- paramSweep_v3(seurat.integrated, PCs = 1:20, sct = FALSE )
...
annotations <- seurat.integrated@meta.data$seurat_clusters
homotypic.prop <- modelHomotypic(annotations)
nExp_poi <- round(0.076*nrow(seurat.integrated@meta.data))
nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))

# run doubletFinder
seurat.integrated <- doubletFinder_v3(seurat.integrated,
                                           PCs = 1:20,
                                           pN = 0.25,
                                           pK = pK,
                                           nExp = nExp_poi.adj,
...
DimPlot(seurat.integrated, reduction = 'umap', group.by = "DF.classifications_0.25_0.19_4283")

dev.off()



seurat.integrated <- subset(seurat.integrated, subset = DF.classifications_0.25_0.19_4283 == "Singlet")
